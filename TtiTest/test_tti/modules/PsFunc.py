...
from config import PS_GPA_KEY, MODE_3, ps_correct_value_power_3mode, correct_value_coefficient_efficiency_3mode, \
    ps_correct_value_coefficient_efficiency_3mode, ps_correct_value_ktc_3mode, URL_ORAWCI


class PsFunc(BaseFunc):
    def __init__(self):
        self.ps_number_measurement_3mode = ''
        self.ps_list_key_params_3mode = ''
    ...
    def adding_measurement_3mode(self):
        # собираем данные
        data_adding_3mode = self.formation_measuremenr_params(PS_GPA_KEY,
                                                              MODE_3)
        # выполняем запрос добавления нового измерения
        result_adding_measurement_ps = session.post('%s' % URL_ORAWCI,
                                                    data_adding_3mode)
        # декодируем приходящий ответ в строку с помощью библиотеки lxml
        decode_result_adding_measurement_ps = self.decode_responce_to_text(result_adding_measurement_ps)
        # вытаскиваем номер добавленного измерения из запроса
        self.ps_number_measurement_3mode = self.get_number_measurement(decode_result_adding_measurement_ps)

    def verify_adding_measurement_3mode(self):
        # проверяем что длина строки > 0, т.е. в ней номер добавленного измерения
        assert len(self.ps_number_measurement_3mode) > 0,\
    ...
    def get_list_key_params_measurement_3mode(self):
        ...
        ps_list_params_key = self.formation_list_params_key(self.ps_number_measurement_3mode)
        result_get_list_params = session.get('%s' % URL_ORAWCI,
                                             params=ps_list_params_key)
        # декодируем приходящий ответ в строку с помощью библиотеки lxml
        ...
        self.ps_list_key_params_3mode = self.get_key_value_measurement(decode_result_get_list_params)
        print len(self.ps_list_key_params_3mode)
    ...
    def save_measurement_3mode(self):
        ...
        data_save_measurement_3mode = self.formation_save_params_ps_3mode(self.ps_number_measurement_3mode,
                                                                          self.ps_list_key_params_3mode)
        ...
        data_for_calculation = self.formation_calculation_params(self.ps_number_measurement_3mode)
    ...
    def updata_gpa_list(self):
        ...
        updata_list_params = self.formation_updata_list_params(PS_GPA_KEY)
    ...
    def verify_calculation_power_ps_3mode(self):
        # достаем значение мощности для добавленного измерения из списка измерений
        power_value_Mvt_3mode = self.get_value_power(self.ps_number_measurement_3mode,
                                                     self.decode_result_updata_list)
        # проверяем что полученное значение равно контрольному
        assert power_value_Mvt_3mode == ps_correct_value_power_3mode,\
            "AssertionError: Power 3 mode wasn't calculated correctly"

    def verify_calculation_coefficient_efficiency_ps_3mode(self):
        # достаем значение коофициента эфективности для добавленного измерения из списка измерений
        coefficient_efficiency_value_3mode = self.get_coefficient_efficiency(self.ps_number_measurement_3mode,
                                                                             self.decode_result_updata_list)
        # проверяем что полученное значение равно контрольному
        assert coefficient_efficiency_value_3mode == ps_correct_value_coefficient_efficiency_3mode,\
            "AssertionError:Coefficient_efficiency 3 mode wasn't calculated correctly"

    def verify_calculation_ktc_ps_3mode(self):
        # достаем значение КТС для добавленного измерения из списка измерений
        ktc_value_3mode = self.get_ktc(self.ps_number_measurement_3mode,
                                       self.decode_result_updata_list)
        # проверяем что полученное значение равно контрольному
        assert ktc_value_3mode == ps_correct_value_ktc_3mode,\
    ...
    def delete_measurement_3mode(self):
        ...
        data_deleting_measurement = self.formation_delete_request(self.ps_number_measurement_3mode)
        # выполняем запрос удаления измерения
        result_delete_measurement_ps = session.post('%s' % URL_ORAWCI,
                                                     data_deleting_measurement)
        # декодируем приходящий ответ в строку с помощью библиотеки lxml
        decode_result_delete_measurement_ps = self.decode_responce_to_text(result_delete_measurement_ps)
        # получаем значения потверждющее удаления
        self.agree_deleted_number_3mode = self.get_key_deleting_measurement(decode_result_delete_measurement_ps)
